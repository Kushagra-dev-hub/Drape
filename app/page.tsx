...
function HomeContent() {
  ...
  async function handleApproveGift(messageId: string, giftId: string) {
    if (!userId) {
      router.push("/login");
      return;
    }
    
    // Find message and filter down to just the approved gift
    const msg = messages.find((m) => m.id === messageId);
    if (!msg || !msg.gifts) return;
    const approvedGift = msg.gifts.find((g) => g.id === giftId);
    if (!approvedGift) return;
    
    const updatedGifts = [approvedGift];
    
    // Update local state immediately
    setMessages((prev) =>
      prev.map((m) => (m.id === messageId ? { ...m, gifts: updatedGifts } : m))
    );
    
    // Update DB
    try {
      await updateMessageGifts(supabase, messageId, updatedGifts);
    } catch (err) {
      console.error("Failed to update approved gift", err);
    }
      ...
      <div className="hero-gradient flex min-h-0 flex-1 flex-col">
        ...
        <main className="flex min-h-0 flex-1 justify-center">
          <div className="flex min-h-0 w-full max-w-3xl flex-col overflow-hidden px-4 py-8">
            {!hasStarted ? (
              <div
                className={`flex flex-1 flex-col items-center justify-center gap-8 pb-16 text-center transition-all duration-300 ease-out ${isHeroExiting ? "scale-95 opacity-0" : "opacity-100"
                  }`}
              >
                ...
                >
                  ...
                  <button
                    ...
                    className="bg-[#1A6B5F] flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-white transition-all hover:opacity-90 active:scale-95 disabled:opacity-30 disabled:hover:opacity-30"
                  >
                    <SendIcon />
                  </button>
                </form>
              ...
              <div className="animate-fade-in mx-auto flex w-full min-h-0 max-w-2xl flex-1 flex-col">
                <div className="scrollbar-hide min-h-0 flex-1 overflow-y-auto flex flex-col gap-5 py-6">
                  {messages.map((m, i) => (
                    <div key={m.id} className="animate-fade-in flex flex-col gap-3" style={{ animationDelay: `${Math.min(i * 0.05, 0.3)}s` }}>
                      <div className={m.role === "user" ? "flex justify-end" : "flex justify-start"}>
                        <div
                          className={
                            m.role === "user"
                              ? "max-w-[70%] whitespace-pre-wrap rounded-2xl rounded-br-md bg-[#1A6B5F] px-4 py-3 text-sm leading-relaxed text-white shadow-md"
                ...
                <div className="mt-2 flex shrink-0 flex-col items-center gap-4 pb-2">
                  ...
                  >
                    ...
                    <button
                      ...
                      aria-label="Send message"
                      disabled={isStreaming || !input.trim()}
                      className="gradient-button flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-white disabled:opacity-30 disabled:hover:shadow-none"
                    >
                      <SendIcon />
                    </button>
                  </form>
